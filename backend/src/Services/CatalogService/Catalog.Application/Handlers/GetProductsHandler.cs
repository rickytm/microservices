...
public class GetProductsHandler : IRequestHandler<GetProductsQuery, PaginationDto<ProductDto>>
{
    ...
    public async Task<PaginationDto<ProductDto>> Handle(GetProductsQuery request, CancellationToken cancellationToken)
    {
        ...
        var results = await _unitOfWork.Repository<Product>().GetAllWithSpec(new ProductSpecification(specParams));
        var totalResults = await _unitOfWork.Repository<Product>().CountAsync(new ProductForCountingSpecification(specParams));

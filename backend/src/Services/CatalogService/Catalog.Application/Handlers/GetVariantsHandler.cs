...
public class GetVariantsHandler : IRequestHandler<GetVariantsQuery, PaginationDto<VariantDto>>
{
    ...
    public async Task<PaginationDto<VariantDto>> Handle(GetVariantsQuery request, CancellationToken cancellationToken)
    {
        ...
        var results = await _unitOfWork.Repository<Variant>().GetAllWithSpec(new VariantSpecification(specParams));
        var totalResults = await _unitOfWork.Repository<Variant>().CountAsync(new VariantForCountingSpecification(specParams));

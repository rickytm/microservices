...
public class GetBrandsHandler : IRequestHandler<GetBrandsQuery, PaginationDto<BrandDto>>
{
    ...
    public async Task<PaginationDto<BrandDto>> Handle(GetBrandsQuery request, CancellationToken cancellationToken)
    {
        ...
        var results = await _unitOfWork.Repository<Brand,Guid>().GetAllWithSpec(new BrandSpecification(specParams));
        var totalResults = await _unitOfWork.Repository<Brand,Guid>().CountAsync(new BrandForCountingSpecification(specParams));
